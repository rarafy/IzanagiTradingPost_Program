    ...
    public class OrderFormatter : MonoBehaviour
    {
        ...
        public void OnSendButtonPushed()
        {
            if (sell1_3.transform.GetChild(0).GetComponent<Text>().text == "" ||
                ...
                buy1.transform.GetChild(2).GetComponent<Text>().text == "")
            {
                Error();
            }
            else
            {
                ...
                //出１ ドロップダウンを初期化
                createOrder.GetComponent<CreateOrder>().Reset();
                //詳細 値を初期化
                ...
                //画面を戻す
                panelSwitchController.GetComponent<PanelSwitchController>().OnTopButtonPressed(GameObject.Find("出品一覧 Button").GetComponent<Button>());
        ...
        public void DataAssignment()
        {
            ...
            sell1_1 = createOrder.GetComponent<CreateOrder>().typeDropdown; itemType = sell1_1;//変数の一貫性を保つため二重代入
